...
class Trainer(BaseTrainer):
    def __init__(self, args, model, architect, loss, resume, config, train_loader, val_loader, test_loader, prefetch=True):
    ...
    def _train_epoch(self, epoch, lr, perturb_alpha, epsilon_alpha):
        if self.args.local_rank == 0: self.logger.info('\n')

        # set epoch for distributed random sampler shuffle
        self.train_loader.loader.sampler.set_epoch(epoch)
        ...
        for batch_idx, (data, target) in enumerate(tbar):
            ...
            if (batch_idx % self.config['trainer']['report_freq'] == 0) and (self.args.local_rank == 0):
    ...
    def _valid_epoch(self, epoch, test, weights_dict=None):
        ...
        if test:
            ...
            phase = 'VALIDATION'
        if self.args.local_rank == 0: self.logger.info(f'\n###### {phase} ######')
