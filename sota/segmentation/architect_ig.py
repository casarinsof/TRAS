...
class Architect(torch.nn.Module):
    def __init__(self, model, args):
        ...
        self.optimizer = torch.optim.Adam(self.model.arch_parameters(),
                                        lr=args['architect']['arch_learning_rate'], betas=(0.5, 0.999),
                                        weight_decay=args['architect']['arch_weight_decay'])
        ...
        #### mode
        if args['arch']['method'] in ['darts', 'darts-proj', 'sdarts', 'sdarts-proj']:
            self.method = 'fo' # first order update
        elif 'so' in args['arch']['method']:
            print('ERROR: PLEASE USE architect.py for second order darts')
        elif args['arch']['method'] in ['blank', 'blank-proj']:
            self.method = 'blank'
        else:
