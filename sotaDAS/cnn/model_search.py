...
class Network(nn.Module):
    ...
    def _initialize_alphas(self):
        k = sum(1 for i in range(self._steps) for n in
                range(1 + i))  # 1 + i instead of 2 + i beacuse I have only one input node per cell not two
        # todo print k
...
if __name__ == '__main__':
    ...
    train_dataset = datasets.MNIST(file_name, train=True, download=True, transform=transform)
    test_dataset = datasets.MNIST(file_name, train=False, download=True, )
