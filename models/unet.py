...
class UNetResnet(BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='resnet50', pretrained=True, freeze_bn=False, freeze_backbone=False, **_):
        ...
        self.convrec = nn.Conv2d(32, 3, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        x1 = self.layer1(self.initial(x)) #4,256,96,96
        x2 = self.layer2(x1) # 4, 512, 48, 48
        x3 = self.layer3(x2) #4, 1024, 48, 48
        x4 = self.layer4(x3) #4, 2048, 48, 48
        
        x = self.upconv1(self.conv1(x4)) # 4, 428, 96, 96
        x = F.interpolate(x, size=(x3.size(2), x3.size(3)), mode="bilinear", align_corners=True) #4, 128, 48, 48
        x = torch.cat([x, x3], dim=1) #4, 1252, 48, 48
        x = self.upconv2(self.conv2(x)) #4, 96, 96, 96

        x = F.interpolate(x, size=(x2.size(2), x2.size(3)), mode="bilinear", align_corners=True) #4, 96, 48, 48
        x = torch.cat([x, x2], dim=1) #4, 608, 48, 48
        x = self.upconv3(self.conv3(x)) #4, 64, 96, 96

        x = F.interpolate(x, size=(x1.size(2), x1.size(3)), mode="bilinear", align_corners=True) #4, 64, 96, 96
        x = torch.cat([x, x1], dim=1) #4, 320, 96, 96

        x = self.upconv4(self.conv4(x)) #4, 48, 192, 192

        x5 = self.upconv5(self.conv5(x)) #4, 32, 384, 384

        # if the input is not divisible by the output stride
        if x.size(2) != H or x.size(3) != W:
            x = F.interpolate(x, size=(H, W), mode="bilinear", align_corners=True)

        x = self.conv7(self.conv6(x5))
        xrec = self.convrec(self.conv6(x5))
        return x, xrec
